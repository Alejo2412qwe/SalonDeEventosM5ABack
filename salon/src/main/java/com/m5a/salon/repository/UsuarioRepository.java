...
//            + "WHERE u.usu_estado = :rol "
//            + "  AND (LOWER(p.per_cedula) LIKE '%:busqueda%' "
//            + "       OR LOWER(u.usu_nombre_usuario) LIKE '%:busqueda%' "
//            + "       OR LOWER(p.per_nombre) LIKE '%:busqueda%' "
//            + "       OR LOWER(p.per_apellido) LIKE '%:busqueda%' "
//            + "       OR LOWER(r.rol_nombre) LIKE '%:busqueda%' "
//            + "      )", nativeQuery = true) 
//    List<Usuario> busquedaU(@Param("busqueda") String busqueda, @Param("rol") int rol);
    @Query(value = "SELECT u.* "
            + "FROM usuario u "
            + "JOIN persona p ON u.usu_per_id = p.per_id "
            + "JOIN rol r ON u.rol_id = r.rol_id "
